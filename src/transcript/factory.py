...
_PLATFORM_PATTERNS: list[tuple[Platform, list[str]]] = [
    ...
    (Platform.FACEBOOK, [
        r"(?:https?://)?(?<!\w)(?:www\.)?facebook\.com/(reel|watch|share)/",
        r"(?:https?://)?fb\.watch/",
    ]),
    (Platform.TIKTOK, [
...
def validate_url(url: str) -> str:
    """Basic URL validation. Returns the URL if valid, raises otherwise."""
    ...
    if parsed.scheme not in ("http", "https"):
        raise TranscriptInvalidURLError(
            f"Unsupported URL scheme '{parsed.scheme}': {url}"
        )
...
def detect_platform_strict(url: str) -> Platform:
    ...
    if platform == Platform.UNKNOWN:
        raise TranscriptUnsupportedPlatformError(
            f"Unsupported video platform for URL: {url}"
        )
    return platform
